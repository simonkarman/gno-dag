...
/**
 * Admin dashboard component. Pure — all instance-specific routing is supplied
 * via props so the same UI can drive the primary and secondary instances:
 *
 * - `apiBase`: API base path (e.g. `/api/admin` or `/api/secondary/admin`).
 * - `pwStorageKey`: sessionStorage key used to persist the password between
 *   reloads. Per-instance so the two admin dashboards do not share creds.
 * - `title`: header label, e.g. `GNO Admin` or `GNO Admin (secondary)`.
 */
export function AdminApp({
  ...
  const fetchRemote = useCallback(async (): Promise<{ data: AdminState; blob: string }> => {
    const res = await fetch(`${apiBase}/state`, { headers: { authorization: `Bearer ${password}` } });
    if (!res.ok) {
      ...
      <div className="min-h-screen flex items-center justify-center bg-zinc-950 text-white">
        <div className="flex flex-col gap-3 w-72">
          ...
          <input
            ...
            onKeyDown={(e) => { if (e.key === 'Enter' && password) { sessionStorage.setItem(pwStorageKey, password); setAuthed(true); } }}
            placeholder="ADMIN_PASSWORD"
            className="rounded bg-zinc-900 border border-zinc-700 px-3 py-2 focus:outline-none focus:border-zinc-400"
          />
          <button
            onClick={() => { sessionStorage.setItem(pwStorageKey, password); setAuthed(true); }}
            disabled={!password}
  ...
  return (
    <div className="h-screen flex flex-col bg-zinc-950 text-white">
      ...
      <header className="flex items-center gap-3 px-4 py-2 border-b border-zinc-800 shrink-0">
        ...
        <button
          onClick={() => { sessionStorage.removeItem(pwStorageKey); setAuthed(false); }}
